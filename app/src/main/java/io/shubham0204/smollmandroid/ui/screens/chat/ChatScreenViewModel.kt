...
class ChatScreenViewModel(
    ...
    private fun sendUserQuery(query: String, addMessageToDB: Boolean = true) {
        ...
        viewModelScope.launch {
            // FIX: try/catch separat + limită 500 caractere pentru prompt
            LOGD("Starting web search for: $query")
            val webContext = try {
                searchWeb(query).take(500)
            } catch (e: Exception) {
                LOGD("searchWeb exception: ${e.message}")
                ""
            }
            ...
            smolLMManager.getResponse(
                ...
                // FIX: onCancelled era gol — acum resetează starea corect
                onCancelled = {
    ...
    // FIX: availMem = memorie liberă, nu folosită. Formula corectă: totalMem - availMem
